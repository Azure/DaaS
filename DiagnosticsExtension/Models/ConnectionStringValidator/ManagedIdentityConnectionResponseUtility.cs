    ...
    public static class ManagedIdentityConnectionResponseUtility
    {
        public static void EvaluateResponseStatus(Exception e, ConnectionStringType type, ref ConnectionStringValidationResult response, string appSettingName = "")
        {
            if (e is ManagedIdentityException)
            {
                ...
                response.Summary = ((ManagedIdentityException)e).MessageSummary;
                response.Details = ((ManagedIdentityException)e).MessageDetails;
            }
            else if (e is UnauthorizedAccessException && e.Message.Contains("unauthorized") || e.Message.Contains("Unauthorized") || e.Message.Contains("request is not authorized"))
            {
                ...
                response.Summary = Constants.AuthorizationFailure;
                if (response.IdentityType == "System")
                {
                    response.Details = String.Format(Constants.SystemAssignedAuthFailure, GetTargetConnectionAppSettingName(type, appSettingName));
                }
                else
                {
                    response.Details = String.Format(Constants.UserAssignedAuthFailure, GetTargetConnectionAppSettingName(type, appSettingName));
                }
            ...
            else if (e is AuthenticationFailedException && e.Message.Contains("ManagedIdentityCredential"))
            {
                ...
                response.Summary = "Your app is configured to use identity based connection but does not have a system assigned managed identity assigned. Refer <a href= 'https://docs.microsoft.com/azure/azure-functions/functions-reference#configure-an-identity-based-connection' target='_blank'>here</a> for details.";
            }
            else if (e.Message.Contains("fullyQualifiedNamespace"))
            {
                ...
                response.Summary = "The app setting " + appSettingName + "__fullyQualifiedNamespace was not found or is set to a blank value. Refer <a href= 'https://docs.microsoft.com/azure/azure-functions/functions-reference#configure-an-identity-based-connection' target='_blank'>here</a> for details.";
            }
                     ...
                     e.InnerException.Message.Contains("The remote name could not be resolved")) // queue and blob
            {
                ...
                response.Summary = String.Format(Constants.StorageAccountResourceNotFound, GetTargetConnectionAppSettingName(type, appSettingName));
            }
            else if (e.Message.Contains("No such host is known")) // event hub and service bus
            {
                response.Status = ConnectionStringValidationResult.ResultStatus.DnsLookupFailed;
                response.Summary = String.Format(Constants.FQNamespaceResourceNotFound, appSettingName);
