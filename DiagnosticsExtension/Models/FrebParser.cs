...
namespace DiagnosticsExtension
{
    class FrebParser
    {
        public async static Task<IEnumerable<FrebFile>> GetFrebFiles()
        {
            ...
            string hostName = Environment.GetEnvironmentVariable("HTTP_HOST");
            ...
            try
            {
                ...
                foreach (var item in tasksList)
                {
                    ...
                    if (frebFile != null)
                    {
                        ...
                        frebFile.Href = GetRelativeLocation(hostName, item.Key.FolderName.Name, item.Key.File.Name);
        ...
        private static string GetRelativeLocation(string hostName, string folder, string fileName)
        {
            return $"https://{hostName}/api/vfs/logfiles/{folder}/{fileName}";
        }
